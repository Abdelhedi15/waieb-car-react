...
import {
  ...
  Hash, BarChart2, StickyNote, ArrowLeft, Printer,
} from 'lucide-react';
...
const CHECKLIST_ITEMS = [
  { key: 'proprete_int',    emoji: '🧹', label: 'Propreté intérieure',      group: 'état' },
  { key: 'proprete_ext',    emoji: '✨', label: 'Propreté extérieure',      group: 'état' },
  { key: 'carburant',       emoji: '⛽', label: 'Niveau carburant correct',  group: 'technique' },
  { key: 'cles',            emoji: '🔑', label: 'Clés rendues',             group: 'documents' },
  { key: 'carte_grise',     emoji: '📄', label: 'Carte grise rendue',       group: 'documents' },
  { key: 'assurance',       emoji: '🛡️', label: 'Vignette assurance',       group: 'documents' },
  { key: 'roue_secours',    emoji: '🔧', label: 'Roue de secours présente', group: 'technique' },
  { key: 'triangles',       emoji: '⚠️', label: 'Triangles de sécurité',    group: 'technique' },
  { key: 'cric',            emoji: '⚙️', label: 'Cric présent',             group: 'technique' },
  { key: 'vitre_ok',        emoji: '🪟', label: 'Vitres intactes',          group: 'carrosserie' },
  { key: 'retroviseurs_ok', emoji: '🔍', label: 'Rétroviseurs intacts',     group: 'carrosserie' },
  { key: 'phares_ok',       emoji: '💡', label: 'Phares/feux fonctionnels', group: 'technique' },
];
...
const GROUP_LABELS = {
  état:        { label: 'État général',  emoji: '🚗' },
  technique:   { label: 'Technique',     emoji: '🔧' },
  carrosserie: { label: 'Carrosserie',   emoji: '🛡️' },
...
<head>
  <meta charset="UTF-8"/>
  <title>Rapport Inspection — ${vehicle?.immatriculation || ''}</title>
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    body{font-family:'Segoe UI',Arial,sans-serif;color:#1A2535;background:white}
    ...
    .header p{font-size:11px;opacity:.7;margin-top:3px}
    ...
    .sig-line{border-top:1.5px solid #CBD5E1;width:160px;margin:40px auto 6px}
...
<div class="header">
  <div>
    <h1>🚗 MTD Group — Waieb Car Rent</h1>
    <p>Rapport d'Inspection de Retour · Document officiel</p>
  </div>
  <div class="header-right">
    <div class="badge">Réservation #${reservation?.id || '—'}</div>
...
<div class="score-banner">
  <div>
    ...
    <div style="font-size:13px;color:#64748B;font-weight:600">Score de retour / 100</div>
  </div>
  <div style="flex:1">
    <div style="font-size:14px;font-weight:800;color:${scoreColor};margin-bottom:4px">
      ${score >= 80 ? '✅ Excellent retour' : score >= 60 ? '⚠️ Retour acceptable avec réserves' : '❌ Dommages à signaler'}
    </div>
    <div style="font-size:11px;color:#64748B;margin-bottom:6px">
      ${eraflures.length} éraflure(s) · ${bosses.length} bosse(s) · Checklist: ${okCheck.length}/${CHECKLIST_ITEMS.length}
    </div>
    <div class="score-bar-wrap"><div class="score-bar"></div></div>
  </div>
  <div style="text-align:right">
    <span style="display:inline-flex;align-items:center;gap:6px;padding:6px 16px;border-radius:20px;font-size:13px;font-weight:800;background:${etatInfo?.bg};color:${etatInfo?.color}">
...
<div class="body">
  <div class="section">
    ...
    <div class="info-grid">
      ...
      <div class="info-box"><div class="lbl">Année</div><div class="val">${vehicle?.annee || '—'}</div></div>
      ...
      <div class="info-box"><div class="lbl">Carburant retour</div><div class="val">${carburant}%</div></div>
  ...
  <div class="section">
    ...
    <div class="checklist-grid">
      ${CHECKLIST_ITEMS.map(item => `
        <div class="check-item ${checklist[item.key] ? 'check-ok' : 'check-ko'}">
          <div class="dot ${checklist[item.key] ? 'dot-ok' : 'dot-ko'}">${checklist[item.key] ? '✓' : '✗'}</div>
          ${item.emoji} ${item.label}
        </div>`).join('')}
    </div>
    ${pbCheck.length > 0 ? `<div style="margin-top:8px;background:#FEF9C3;border:1px solid #FDE68A;border-radius:8px;padding:8px 12px;font-size:11.5px;color:#92580A;font-weight:600">
      ⚠️ Points à signaler : ${pbCheck.map(i => i.label).join(', ')}
    </div>` : ''}
  </div>
  ...
  <div class="section">
    ...
    ${marks.length === 0
      ...
      : `<div class="marks-list">
          ${eraflures.map(m => `<span class="mark-chip mark-eraflure">— Éraflure · ${m.label}</span>`).join('')}
          ${bosses.map(m    => `<span class="mark-chip mark-bosse">● Bosse · ${m.label}</span>`).join('')}
         </div>`}
  </div>
      ...
      <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.65)', backdropFilter:'blur(4px)',
        ...
        <div style={{ background:'white', borderRadius:'20px', width:'100%', maxWidth:'500px',
          ...
          <div style={{ background:'linear-gradient(135deg,#16A34A,#15803D)', padding:'24px', color:'white', textAlign:'center' }}>
            ...
            <p style={{ margin:'6px 0 0', opacity:.8, fontSize:'13px' }}>
              {vehicle?.marque} {vehicle?.modele} · {vehicle?.immatriculation}
            </p>
          </div>

          <div style={{ padding:'20px 24px' }}>
            <div style={{ background: score >= 80 ? '#F0FFF4' : score >= 60 ? '#FFFBEB' : '#FFF5F5',
              border:`2px solid ${scoreColor}`, borderRadius:'14px', padding:'16px', textAlign:'center', marginBottom:'16px' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'8px', marginBottom:'20px' }}>
              {[
                { emoji:'📋', val:`${CHECKLIST_ITEMS.length - nbPbChecklist}/${CHECKLIST_ITEMS.length}`, label:'Checklist' },
                { emoji:'🛡️', val:nbMarks,      label:'Dommages' },
                { emoji:'⛽', val:`${carburant}%`, label:'Carburant' },
              ].map(s => (
                <div key={s.label} style={{ background:'#F8FAFC', borderRadius:'8px', padding:'10px', textAlign:'center', border:'1px solid #E2E8F0' }}>
                  <div style={{ fontSize:'18px', marginBottom:'3px' }}>{s.emoji}</div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
              <button onClick={handlePrint}
                style={{ width:'100%', padding:'13px', background:NAVY, color:'white', border:'none',
                  borderRadius:'12px', cursor:'pointer', fontWeight:'800', fontSize:'14px',
                  display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' }}>
                🖨️ Imprimer le rapport d'inspection
              </button>
              <button onClick={onClose}
                style={{ width:'100%', padding:'11px', background:'#F8FAFC', color:'#64748B',
                  border:'1.5px solid #DDE3ED', borderRadius:'12px', cursor:'pointer', fontWeight:'700', fontSize:'13px' }}>
                Fermer
              </button>
            </div>
          </div>
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.65)', backdropFilter:'blur(4px)',
      ...
      <div style={{ background:'white', borderRadius:'20px', width:'100%', maxWidth:'680px',
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'11px' }}>
              ...
              <div>
                <h2 style={{ margin:0, fontSize:'16px', fontWeight:'800', display:'flex', alignItems:'center', gap:'7px' }}>
                  <Eye size={16}/> Inspection de Retour
                </h2>
          ...
          <div style={{ display:'flex', gap:'5px', marginTop:'14px' }}>
            {steps.map((s, i) => (
              <div key={i} style={{ display:'flex', alignItems:'center', flex:1 }}>
                <div style={{ flex:1, display:'flex', alignItems:'center', gap:'6px', padding:'5px 10px', borderRadius:'20px',
                  ...
                  <div style={{ width:'20px', height:'20px', borderRadius:'50%', flexShrink:0,
                    ...
                    display:'flex', alignItems:'center', justifyContent:'center', fontWeight:'800', fontSize:'10px' }}>
                    {step > i+1 ? '✓' : i+1}
                  </div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'18px 22px' }}>
          ...
          {step === 1 && (
            <div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'16px' }}>
                {/* Km */}
                <div style={{ background:'#F8FAFC', borderRadius:'12px', padding:'13px',
                  border: !kilometrage ? '2px solid #FECACA' : '2px solid #86EFAC' }}>
                  <label style={{ fontSize:'11.5px', fontWeight:'700', color:'#64748B', marginBottom:'5px', display:'flex', alignItems:'center', gap:'5px' }}>
                    📍 Kilométrage retour <span style={{ color:RED, fontWeight:'900' }}>*</span>
                  </label>
                  <input type="number" value={kilometrage} onChange={e => setKilometrage(e.target.value)}
                    placeholder={`Actuel: ${vehicle?.kilometrage || '—'} km`}
                    style={{ width:'100%', padding:'9px', border:`1.5px solid ${!kilometrage ? '#FECACA' : '#DDE3ED'}`,
                      borderRadius:'8px', fontSize:'14px', fontWeight:'700', boxSizing:'border-box' }}/>
                  {!kilometrage && (
                    <div style={{ fontSize:'10.5px', color:RED, fontWeight:'600', marginTop:'4px' }}>
                      ⚠️ Champ obligatoire
                    </div>
                  )}
                </div>
                ...
                <div style={{ background:'#F8FAFC', borderRadius:'12px', padding:'13px', border:'1.5px solid #E2E8F0' }}>
                  <label style={{ fontSize:'11.5px', fontWeight:'700', color:'#64748B', marginBottom:'5px', display:'flex', alignItems:'center', gap:'5px' }}>
                    ⛽ Carburant — <span style={{ color: carburant < 25 ? RED : carburant < 50 ? AMBER : GREEN, fontWeight:'800' }}>{carburant}%</span>
                  </label>
                    ...
                    {CHECKLIST_ITEMS.filter(i => i.group === group).map(item => {
                      ...
                      return (
                        <div key={item.key}
                          ...
                          style={{ display:'flex', alignItems:'center', gap:'8px', padding:'8px 12px', borderRadius:'9px', cursor:'pointer',
                            border:`1.5px solid ${ok ? '#86EFAC' : '#FECACA'}`,
                            background: ok ? '#F0FFF4' : '#FFF5F5', transition:'all 0.14s' }}>
                          ...
                          <div style={{ width:'20px', height:'20px', borderRadius:'50%', flexShrink:0,
                            background: ok ? GREEN : RED, display:'flex', alignItems:'center', justifyContent:'center' }}>
                            <span style={{ color:'white', fontSize:'10px', fontWeight:'900' }}>{ok ? '✓' : '✗'}</span>
                          </div>
                        </div>
                      );
                    })}
              ...
              {nbPbChecklist > 0 && (
                <div style={{ background:'#FEF9C3', border:'1px solid #FDE68A', borderRadius:'9px', padding:'9px 12px',
                  fontSize:'12px', color:'#92580A', fontWeight:'600', display:'flex', alignItems:'center', gap:'6px', marginTop:'4px' }}>
                  ⚠️ {nbPbChecklist} point(s) à signaler
                </div>
              )}
            </div>
          )}
            ...
            <div>
                ...
                {Object.entries(CAR_MARKS).map(([type, cfg]) => (
                  <button key={type} onClick={() => setMarkType(type)}
                    style={{ flex:1, padding:'9px', borderRadius:'10px', cursor:'pointer',
                      ...
                      fontWeight:'800', fontSize:'12.5px', display:'flex', alignItems:'center', justifyContent:'center', gap:'5px' }}>
                    <span style={{ fontSize:'15px' }}>{cfg.symbol}</span> {cfg.label}
                  </button>
                ))}
              </div>

              <div style={{ background:'#F8FAFC', borderRadius:'14px', padding:'13px', marginBottom:'13px', border:'1.5px dashed #DDE3ED' }}>
                <div style={{ fontSize:'11px', color:'#64748B', textAlign:'center', marginBottom:'9px', fontWeight:'600' }}>
                ...
                <div style={{ display:'flex', justifyContent:'center', gap:'18px', marginTop:'9px' }}>
                  {Object.entries(CAR_MARKS).map(([type, cfg]) => (
                    <span key={type} style={{ display:'flex', alignItems:'center', gap:'4px', fontSize:'11px', fontWeight:'700', color:cfg.color }}>
              ...
              {marks.length > 0 && (
                <div style={{ background:'#FEF9C3', border:'1px solid #FDE68A', borderRadius:'9px', padding:'10px', marginBottom:'13px' }}>
                  <div style={{ fontSize:'11.5px', fontWeight:'800', color:'#92580A', marginBottom:'6px' }}>
                    ⚠️ {marks.length} dommage(s) marqué(s)
                  </div>
                  <div style={{ display:'flex', gap:'5px', flexWrap:'wrap' }}>
                    {marks.map((m, i) => {
                      const cfg = CAR_MARKS[m.type];
                      return (
                        <span key={i} style={{ background:'white', border:`1.5px solid ${cfg.color}`, color:cfg.color,
              ...
              <div>
                <div style={{ fontSize:'10.5px', fontWeight:'800', color:'#64748B', marginBottom:'8px', textTransform:'uppercase', letterSpacing:'0.5px' }}>
                  🛡️ État général
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'7px' }}>
                  {ETAT_OPTIONS.map(opt => (
                    <div key={opt.value} onClick={() => setEtatGeneral(opt.value)}
                      style={{ textAlign:'center', padding:'11px 6px', borderRadius:'11px', cursor:'pointer',
                        border:`2px solid ${etatGeneral === opt.value ? opt.color : '#DDE3ED'}`,
                        background: etatGeneral === opt.value ? opt.bg : 'white', transition:'all 0.14s' }}>
          ...
          {step === 3 && (
            <div>
              <div style={{ background:`linear-gradient(135deg,${scoreColor}22,white)`,
                border:`2px solid ${scoreColor}`, borderRadius:'14px', padding:'16px', textAlign:'center', marginBottom:'16px' }}>
                <div style={{ fontSize:'52px', fontWeight:'900', color:scoreColor, lineHeight:1 }}>{score}</div>
                <div style={{ fontSize:'12.5px', fontWeight:'700', color:scoreColor, marginTop:'4px' }}>
                  Score de Retour / 100
                </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'8px', marginBottom:'13px' }}>
                {[
                  { emoji:'📋', label:'Checklist', value:`${CHECKLIST_ITEMS.length - nbPbChecklist}/${CHECKLIST_ITEMS.length}`, ok: nbPbChecklist === 0 },
                  { emoji:'🛡️', label:'Dommages',  value: nbMarks, ok: nbMarks === 0 },
                  { emoji:'🚗', label:'État',       value: ETAT_OPTIONS.find(o => o.value === etatGeneral)?.label, ok: etatGeneral === 'excellent' || etatGeneral === 'bon' },
                ].map(item => (
                  <div key={item.label} style={{ textAlign:'center', padding:'12px', borderRadius:'11px',
                    background: item.ok ? '#F0FFF4' : '#FFF5F5',
              ...
              {kilometrage && (
                <div style={{ background:'#EFF4FB', borderRadius:'9px', padding:'10px 13px', marginBottom:'11px',
                  display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <span style={{ fontSize:'12px', color:'#64748B', fontWeight:'600' }}>📍 Kilométrage retour</span>
              ...
              <div>
                ...
                <textarea value={notes} onChange={e => setNotes(e.target.value)}
                  placeholder="Décrivez l'état général, dommages observés, remarques du client..."
                  rows={4}
                  style={{ width:'100%', padding:'10px', border:'1.5px solid #DDE3ED', borderRadius:'9px',
                    fontSize:'12.5px', resize:'vertical', fontFamily:'inherit', boxSizing:'border-box' }}/>
              </div>
            </div>
          )}
        </div>
        ...
        {/* ── Footer */}
        <div style={{ padding:'13px 22px', borderTop:'1px solid #F0F2F5', display:'flex', justifyContent:'space-between',
          alignItems:'center', background:'#F8FAFC', flexShrink:0 }}>
          <div style={{ fontSize:'11.5px', color:'#64748B', fontWeight:'600', display:'flex', alignItems:'center', gap:'5px' }}>
            Étape {step}/{steps.length}
            {step === 1 && !kilometrage && <span style={{ color:RED, marginLeft:'7px', fontWeight:'700' }}>⚠️ km obligatoire</span>}
            {step === 1 && nbPbChecklist > 0 && <span style={{ color:AMBER, marginLeft:'7px' }}>⚠️ {nbPbChecklist} point(s)</span>}
            {step === 2 && nbMarks > 0 && <span style={{ color:RED, marginLeft:'7px' }}>💥 {nbMarks} dommage(s)</span>}
            {step === 3 && <span style={{ color:scoreColor, marginLeft:'7px', fontWeight:'800' }}>Score: {score}/100</span>}
          </div>
          <div style={{ display:'flex', gap:'8px' }}>
            {step > 1 && (
              <button onClick={() => setStep(s => s - 1)}
                style={{ padding:'8px 16px', background:'white', color:NAVY, border:`1.5px solid ${NAVY}`,
                  borderRadius:'9px', cursor:'pointer', fontWeight:'700', fontSize:'12.5px',
            ...
            {step < 3 ? (
              <button onClick={() => setStep(s => s + 1)}
                style={{ padding:'8px 20px', background:NAVY, color:'white', border:'none', borderRadius:'9px',
                  cursor:'pointer', fontWeight:'700', fontSize:'12.5px', display:'flex', alignItems:'center', gap:'4px' }}>
                Suivant →
              </button>
            ) : (
              <button onClick={handleSubmit} disabled={submitting}
                style={{ padding:'8px 20px', background:GREEN, color:'white', border:'none', borderRadius:'9px',
                  cursor: submitting ? 'not-allowed' : 'pointer', fontWeight:'800', fontSize:'12.5px',
                  display:'flex', alignItems:'center', gap:'4px', opacity: submitting ? 0.7 : 1 }}>
