...
const IMMAT_PHOTOS = {'240TN5082':'https://i.ibb.co/FZmVWK6/vec1.jpg','259TN5651':'https://i.ibb.co/F4SbDBMM/vec2.jpg','243TN1422':'https://i.ibb.co/gbw2JtTH/vec3.jpg','236TN5648':'https://i.ibb.co/0RJ31jBB/vec4.jpg','234TN2126':'https://i.ibb.co/prkyKtjv/vec5.jpg','244TN7005':'https://i.ibb.co/P81vS80/vec6.jpg','251TN1694':'https://i.ibb.co/5WBKGTGL/vec7.jpg','252TN3310':'https://i.ibb.co/9kNtVZGB/vec8.png','253TN4421':'https://i.ibb.co/jvRzYcDB/vec9.png','254TN6632':'https://i.ibb.co/hxvysSY4/vec10.png','255TN7743':'https://i.ibb.co/dsfz2VnP/vec11.png','256TN8854':'https://i.ibb.co/35ccmkFY/vec12.jpg','257TN1301':'https://i.ibb.co/zTqsrTvT/vec13.jpg','258TN1402':'https://i.ibb.co/hRDFJYZy/vec14.jpg','259TN1503':'https://i.ibb.co/n8DyCGCr/vec15.jpg','260TN1604':'https://i.ibb.co/wZfGy948/vec16.jpg','261TN1705':'https://i.ibb.co/ZzNTcc8G/vec17.jpg','262TN1806':'https://i.ibb.co/v6Bqz1Vm/vec18.jpg','263TN1907':'https://i.ibb.co/KzVK2JNw/vec19.jpg','264TN2008':'https://i.ibb.co/JFkFWp6g/vec20.jpg','265TN2109':'https://i.ibb.co/1fLYRnVC/vec21.jpg','266TN2210':'https://i.ibb.co/d0SK6j6H/vec22.jpg'};
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '6px', marginBottom: '12px' }}>
        {[
          ...
          { label: 'RESTANT',  value: isSolde ? 'Soldé' : `${solde.montant_restant} DT`,
            color: isSolde ? GREEN : AMBER,
            bg: isSolde ? '#DCFCE7' : '#FEF9C3',
            border: isSolde ? GREEN : AMBER },
        ].map(s => (
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start' }}>
        {steps.map((step, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
            ...
            {i < steps.length - 1 && (
              <div style={{ height: '2px', width: '20px', flexShrink: 0, background: step.paid ? NAVY : '#E2E8F0', marginBottom: '18px' }} />
            )}
          </div>
        ))}
      </div>
...
const AccidentModal = ({ reservation, client, vehicle, vehicles, onClose, onConfirm }) => {
  ...
  const [step, setStep]                          = useState(1); // 1=notify 2=choose vehicle 3=confirm
  ...
  const extraCost     = extraDays > 0 && selectedVehicle
    ? (extraDays * parseFloat(selectedVehicle.prix_journalier)).toFixed(2)
    : '0.00';
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '620px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '20px' }}>
          {['Notification client','Choix véhicule','Confirmation'].map((s, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px', flex: 1 }}>
              <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: step > i ? NAVY : step === i+1 ? AMBER : '#E2E8F0',
                color: step >= i+1 ? 'white' : '#94A3B8', display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        {step === 1 && (
          <>
            ...
            <div style={{ background: '#FEE2E2', border: '1px solid #FECACA', borderRadius: '10px', padding: '14px', marginBottom: '16px', fontSize: '13px' }}>
              ...
              <div style={{ color: '#7F1D1D' }}>Véhicule concerné: <strong>{vehicle?.marque} {vehicle?.modele}</strong> ({vehicle?.immatriculation})</div>
            ...
            {client && (
              <div style={{ background: '#F8FAFC', border: '1px solid #DDE3ED', borderRadius: '10px', padding: '14px', marginBottom: '16px' }}>
                <div style={{ fontWeight: '700', color: NAVY, marginBottom: '10px', fontSize: '13px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                  Client à notifier
                </div>
        ...
        {/* ── Step 2: Choose replacement vehicle */}
        {step === 2 && (
          <>
            ...
            {loading ? (
              ...
              <div>
                <div style={{ fontSize: '12px', color: '#64748B', fontWeight: '600', marginBottom: '10px' }}>
                  {availableVehicles.length} véhicule(s) similaire(s) disponible(s) (prix ±30%)
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '280px', overflowY: 'auto', paddingRight: '4px' }}>
                  {availableVehicles.map(v => {
                    const isSelected = selectedVehicle?.id === v.id;
                    const days = newDays;
                    const total = (days * parseFloat(v.prix_journalier)).toFixed(2);
                    return (
                      <div key={v.id} onClick={() => setSelectedVehicle(v)}
                        style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', border: `2px solid ${isSelected ? NAVY : '#DDE3ED'}`, borderRadius: '10px', cursor: 'pointer', background: isSelected ? '#EFF4FB' : 'white', transition: 'all 0.14s' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: '12px', color: '#64748B' }}>{v.immatriculation} · {v.nombre_places} places</div>
                        </div>
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          <div style={{ fontWeight: '800', color: GREEN, fontSize: '14px' }}>{v.prix_journalier} DT/j</div>
                          <div style={{ fontSize: '11.5px', color: PURPLE, fontWeight: '600' }}>{days}j → {total} DT</div>
                        </div>
            ...
            {extraDays > 0 && selectedVehicle && (
              <div style={{ background: '#FEF3DC', border: '1px solid #E8A020', borderRadius: '8px', padding: '10px 14px', marginTop: '12px', fontSize: '13px', color: '#92580A', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <>
            ...
            <div className="modal-actions">
              ...
              <button className="btn btn-primary" style={{ background: GREEN, display: 'flex', alignItems: 'center', gap: '6px' }}
                onClick={() => onConfirm(selectedVehicle, newDateFin, reservation)}>
                <CheckCircle size={15} /> Confirmer le remplacement
              </button>
            </div>
          </>
        )}
    ...
    <div style={{ background: '#FEF3DC', border: `2px solid ${AMBER}`, borderRadius: '14px', padding: '16px 20px', marginBottom: '22px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <div style={{ fontWeight: '800', fontSize: '14px', color: '#1A2535' }}>
              📱 Nouvelles réservations — Application Mobile
            </div>
            <div style={{ fontSize: '12px', color: '#92580A' }}>
              {pendingReservations.length} demande(s) en attente de confirmation admin
            </div>
          </div>
        </div>
        ...
        {pendingReservations.map(r => {
          ...
          return (
            <div key={r.id} style={{ background: 'white', borderRadius: '12px', padding: '14px 16px', border: '1.5px solid #FCD34D', display: 'grid', gridTemplateColumns: '60px 1fr auto auto', alignItems: 'center', gap: '14px' }}>
              {/* Vehicle photo */}
              <img src={getCarPhoto(vehicle)} alt={vehicle?.marque || ''}
                style={{ width: '60px', height: '44px', objectFit: 'cover', borderRadius: '8px' }}
                onError={e => { e.target.src = CAR_PHOTOS.default; }} />
              ...
              <div>
                ...
                <div style={{ fontSize: '13px', fontWeight: '600', color: '#1A2535' }}>
                ...
                <div style={{ fontSize: '12px', color: '#64748B', display: 'flex', gap: '14px', marginTop: '2px', flexWrap: 'wrap' }}>
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <CalendarDays size={11} /> {r.date_debut} → {r.date_fin} ({days}j)
                  </span>
                </div>
              </div>
...
const ReservationsList = () => {
  ...
  const fetchSoldes = async (list) => {
    ...
    await Promise.all(list.map(async r => {
      try { const res = await api.get(`/payments/solde/${r.id}/`); map[r.id] = res.data; }
      catch {}
    }));
    setSoldes(map);
  };
  ...
  useEffect(() => {
    fetchAll();
    const interval = setInterval(fetchAll, 10000); // ← 10s for near real-time
    return () => clearInterval(interval);
  }, [fetchAll]);

  const handleRefresh = async () => { setRefreshing(true); await fetchAll(); setRefreshing(false); };

  // ── Accept or refuse a reservation from mobile
  ...
  // ── Confirm replacement
  const handleConfirmReplacement = async (newVehicle, newDateFin, reservation) => {
    try {
      await api.patch(`/reservations/${reservation.id}/`, {
        ...
        montant_total: (
          Math.max(1, Math.round((new Date(newDateFin) - new Date(reservation.date_debut)) / 86400000)) *
          parseFloat(newVehicle.prix_journalier)
        ).toFixed(2),
      });
      await fetchAll();
  ...
  return (
    <div>
      ...
      {/* ── Mobile notification panel — shows en_attente reservations */}
      <MobileNotifPanel
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: '220px' }}>
            ...
            <input value={search} onChange={e => handleSearch(e.target.value)}
              placeholder="Rechercher par client, CIN, véhicule..."
            ...
            <div key={r.id} style={{ background: 'white', borderRadius: '14px', overflow: 'hidden',
              ...
              <div style={{ padding: '12px 20px', borderBottom: '1px solid #F0F2F5', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                  ...
                  {contract && (
                    <span style={{ color: PURPLE, fontWeight: '700', fontSize: '12.5px', background: '#F3EEFF', padding: '2px 8px', borderRadius: '6px' }}>
                      {contract.numero}
                    </span>
                  )}
                  ...
                  {r.a_accident && (
                    <span style={{ background: '#FEE2E2', color: RED, padding: '3px 10px', borderRadius: '10px', fontSize: '12px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <AlertTriangle size={12} /> Accident
                    </span>
                  )}
                  {r.vehicule_remplace && (
                    <span style={{ background: '#DCFCE7', color: GREEN, padding: '3px 10px', borderRadius: '10px', fontSize: '12px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <CheckCircle size={12} /> Véhicule remplacé
                    </span>
                  )}
                  {isSolde && (
                    <span style={{ background: '#DCFCE7', color: GREEN, padding: '3px 10px', borderRadius: '10px', fontSize: '12px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  {/* Accident action button */}
                  {hasAccidentNoReplacement && (
                    <button
                      onClick={() => setAccidentModal({ reservation: r, client, vehicle })}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1.4fr' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                        {client.cin && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <span style={{ background: '#EFF4FB', padding: '1px 7px', borderRadius: '4px', color: NAVY, fontWeight: '700', fontSize: '11px' }}>CIN</span>
                            <span style={{ fontWeight: '700', color: '#1A2535', fontSize: '13px' }}>{client.cin}</span>
                          </div>
                        )}
                  ...
                  {vehicle ? (
                    <>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        ...
                        <div style={{ fontSize: '13px', color: GREEN, fontWeight: '700', display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <Banknote size={12} /> {vehicle.prix_journalier} DT/jour
                        </div>
                        <div style={{ fontSize: '12px', color: PURPLE, fontWeight: '600' }}>
                          {days}j × {vehicle.prix_journalier} = {(parseFloat(vehicle.prix_journalier) * days).toFixed(2)} DT
                        </div>
                      </div>
                    </>
                  ) : <span style={{ color: '#94A3B8', fontSize: '13px' }}>—</span>}
                ...
                <div style={{ padding: '16px 20px' }}>
                  <div style={{ fontSize: '10.5px', fontWeight: '800', color: AMBER, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                    <Banknote size={12} /> Situation Financière
                  </div>
