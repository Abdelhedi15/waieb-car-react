  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setAvailableVehicles(res.data.filter(v => v.id !== reservation.vehicle && parseFloat(v.prix_journalier) >= prixRef * 0.7 && parseFloat(v.prix_journalier) <= prixRef * 1.3));
      } catch { setAvailableVehicles([]); } finally { setLoading(false); }
    })();
  }, [reservation, vehicle]);
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '20px' }}>
          {['Notification client', 'Choix véhicule', 'Confirmation'].map((s, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px', flex: 1 }}>
              <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: step > i ? NAVY : step === i+1 ? AMBER : '#E2E8F0', color: step >= i+1 ? 'white' : '#94A3B8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '800', fontSize: '12px', flexShrink: 0 }}>{step > i+1 ? '✓' : i+1}</div>
    ...
    <div style={{ background: '#FEF3DC', border: `2px solid ${AMBER}`, borderRadius: '14px', padding: '16px 20px', marginBottom: '22px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
        <div style={{ width: '36px', height: '36px', background: AMBER, borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          ...
          <span style={{ position: 'absolute', top: '-6px', right: '-6px', background: RED, color: 'white', borderRadius: '50%', width: '18px', height: '18px', fontSize: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '800' }}>{pendingReservations.length}</span>
        </div>
        <div>
          ...
          return (
            <div key={r.id} style={{ background: 'white', borderRadius: '12px', padding: '14px 16px', border: '1.5px solid #FCD34D', display: 'grid', gridTemplateColumns: '60px 1fr auto auto', alignItems: 'center', gap: '14px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={() => handleAction(r.id, 'annulée')} disabled={!!isProcessing} style={{ padding: '8px 14px', background: '#FEE2E2', color: RED, border: `1.5px solid ${RED}`, borderRadius: '8px', cursor: 'pointer', fontWeight: '700', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '5px', opacity: isProcessing ? 0.6 : 1 }}>
                  <XCircle size={14} />{isProcessing === 'annulée' ? '...' : 'Refuser'}
                </button>
                <button onClick={() => handleAction(r.id, 'confirmée')} disabled={!!isProcessing} style={{ padding: '8px 14px', background: GREEN, color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '700', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '5px', opacity: isProcessing ? 0.6 : 1 }}>
...
const ReservationsList = () => {
  ...
  const handleRetourConfirm = async (data) => {
    try {
      await api.patch(`/reservations/${data.reservation_id}/`, {
        inspection_retour_faite: true, statut: 'terminée',
        etat_retour: data.etat_retour, notes_retour: data.notes_retour,
        score_retour: data.score_retour, kilometrage_retour: data.kilometrage_retour,
        carburant_retour: data.carburant_retour,
        eraflures_retour: data.eraflures_retour, bosses_retour: data.bosses_retour,
      });
      try {
  ...
  const handleConfirmReplacement = async (newVehicle, newDateFin, reservation, client) => {
    try {
      ...
      await api.patch(`/reservations/${reservation.id}/`, { vehicle: newVehicle.id, date_fin: newDateFin, vehicule_remplace: reservation.vehicle, raison_remplacement: 'Accident — véhicule indisponible', montant_total: newTotal, acompte: (parseFloat(newTotal) * acomptePct).toFixed(2) });
      if (client?.email) {
        try { await api.post('/vehicles/send-replacement-email/', { client_email: client.email, client_nom: client.nom, client_prenom: client.prenom, ancien_vehicule: `${oldVeh?.marque} ${oldVeh?.modele} (${oldVeh?.immatriculation})`, nouveau_vehicule: `${newVehicle.marque} ${newVehicle.modele} (${newVehicle.immatriculation})`, date_debut: reservation.date_debut, date_fin: newDateFin, reservation_id: reservation.id, nouveau_total: newTotal }); }
            ...
            <div key={r.id} style={{
              ...
              <div style={{ padding: '12px 20px', borderBottom: '1px solid #F0F2F5', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px', background: needsRetour ? '#FAF5FF' : isSolde ? '#F0FFF4' : r.statut === 'confirmée' ? '#EFF4FB' : r.statut === 'annulée' ? '#FFF5F5' : '#FFFBEB' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                  ...
                  {r.inspection_retour_faite && <span style={{ background: '#EDE9FE', color: PURPLE, padding: '3px 10px', borderRadius: '10px', fontSize: '12px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '4px' }}><ClipboardList size={12}/> Inspecté {r.score_retour !== undefined ? `· ${r.score_retour}/100` : ''}</span>}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                  {needsRetour && (
                    <button onClick={() => setRetourModal({ reservation: r, client, vehicle })}
                      style={{ padding: '7px 14px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '800', fontSize: '12.5px', display: 'flex', alignItems: 'center', gap: '6px', background: `linear-gradient(135deg, ${PURPLE}, #5B21B6)`, color: 'white', boxShadow: '0 3px 10px rgba(124,58,237,0.3)', animation: 'pulse 2s infinite' }}>
