...
import {
  ...
  Banknote, Users, Gauge, ArrowRight, ClipboardCheck,
} from 'lucide-react';
import Calendar from 'react-calendar';
import 'react-calendar/dist/Calendar.css';
import api from '../api/axios';
import './Reservations.css';
import RetourCheckModal from '../components/RetourCheckModal';
...
const getPrixSaison = (vehicle, dateDebut) => {
  ...
  if ([7,8].includes(mois)) return parseFloat(vehicle.prix_tres_haute_saison || parseFloat(vehicle.prix_journalier)*1.5);
  if ([6,9].includes(mois)) return parseFloat(vehicle.prix_haute_saison    || parseFloat(vehicle.prix_journalier)*1.25);
...
const checkClientConflict = (clientId, dateDebut, dateFin, reservations, excludeId=null) => {
  ...
  return reservations.find(r => {
    ...
    if (r.client !== parseInt(clientId)) return false;
    if (!['confirmée','en_attente'].includes(r.statut)) return false;
    return r.date_debut <= dateFin && r.date_fin >= dateDebut;
  }) || null;
};
...
const checkVehicleConflict = (vehicleId, dateDebut, dateFin, reservations, excludeId=null) => {
  ...
  return reservations.find(r => {
    if (excludeId && r.id === excludeId) return false;
    if (r.vehicle !== parseInt(vehicleId)) return false;
    if (!['confirmée','en_attente'].includes(r.statut)) return false;
    return r.date_debut <= dateFin && r.date_fin >= dateDebut;
  }) || null;
};
...
const MiniPagination = ({ currentPage, totalPages, onPageChange, totalItems, perPage }) => {
  ...
  const from=(currentPage-1)*perPage+1, to=Math.min(currentPage*perPage,totalItems);
  return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:'16px',paddingTop:'14px',borderTop:'1px solid #DDE3ED'}}>
      <div style={{fontSize:'12px',color:'#64748B'}}><strong style={{color:'#1A2535'}}>{from}–{to}</strong> sur <strong style={{color:'#1A2535'}}>{totalItems}</strong> véhicules</div>
...
const Reservations = () => {
  ...
  const fetchAll = async () => {
    try {
      const [r,c,v] = await Promise.all([api.get('/reservations/'),api.get('/clients/'),api.get('/vehicles/')]);
  ...
  const setFormAndCheck = (newForm) => {
    setForm(newForm);
    updateConflicts(newForm, reservations);
  };

  const handleInspectionConfirm = async (payload) => {
    if (!inspectionRes) throw new Error('Réservation manquante');
    await api.patch(`/reservations/${inspectionRes.id}/`, payload);
    const vehicleId = inspectionRes.vehicle;
    const etatCar = payload.etat_retour==='excellent'||payload.etat_retour==='bon' ? 'excellent'
      : payload.etat_retour==='defauts' ? 'defauts' : 'dommages';
    await api.patch(`/vehicles/${vehicleId}/`, { statut:'disponible', etat_carrosserie:etatCar, kilometrage:payload.kilometrage_retour });
    await fetchAll();
  };

  const openInspection = (r) => { setInspectionRes(r); setShowInspection(true); };

  const getUpcomingReservations = () => {
    const today=new Date();today.setHours(0,0,0,0);
    return reservations
      .filter(r=>r.statut!=='annulée'&&r.statut!=='terminée')
      .map(r=>{const debut=new Date(r.date_debut);debut.setHours(0,0,0,0);return{...r,daysLeft:Math.round((debut-today)/86400000)};})
  ...
  const bookedDates = (() => {
    ...
    reservations.forEach(r => {
      ...
      let cur=new Date(r.date_debut);const end=new Date(r.date_fin);
  ...
  const checkAvailability = async () => {
    ...
    setCheckingAvailability(true); setVehiclePage(1); setVehicleSearch('');
    try {
      ...
      setAvailableVehicles(res.data); setShowCalendar(true);
    } catch(err){console.error(err);}
  ...
  const filteredVehicles = availableVehicles
    ...
    .sort((a,b) => {
      const d1=calendarDates[0]?.toISOString().split('T')[0];
      const pA=getPrixSaison(a,d1), pB=getPrixSaison(b,d1);
  ...
  const paginatedVehicles=filteredVehicles.slice((vehiclePage-1)*VEHICLES_PER_PAGE,vehiclePage*VEHICLES_PER_PAGE);

  const openReserveFromCalendar = (vehicle) => {
    const d1=calendarDates[0].toISOString().split('T')[0];
    const d2=calendarDates[1].toISOString().split('T')[0];
    const days=calcDays(d1,d2), prix=getPrixSaison(vehicle,d1);
    const total=(days*prix).toFixed(2);
    const rule=getAcompteRule(days);
    const acompteMin=(parseFloat(total)*rule.pct/100).toFixed(2);
    const newForm={...EMPTY_FORM,vehicle:String(vehicle.id),date_debut:d1,date_fin:d2,montant_total:total,acompte:acompteMin};
    setEditingRes(null); setFromCalendar(true);
    setFormAndCheck(newForm);
    setShowModal(true);
  };
  ...
  const calcTotal = (debut, fin, vehicleId) => {
    ...
    if(!v) return;
    const days=calcDays(debut,fin), prix=getPrixSaison(v,debut);
    const total=(days*prix).toFixed(2);
    const rule=getAcompteRule(days);
    const acompteMin=(parseFloat(total)*rule.pct/100).toFixed(2);
    setForm(f=>{
      const nf={...f,montant_total:total,acompte:acompteMin};
      updateConflicts(nf,reservations);
      return nf;
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fetchAll(); setShowModal(false); setShowCalendar(false);
  ...
  const confirmerRemplacement = async (vehicleId, raison) => {
    try {
      await api.patch(`/reservations/${remplacementReservation.id}/`,{vehicle:vehicleId,vehicule_remplace:remplacementReservation.vehicle,raison_remplacement:raison,statut:'confirmée'});
      fetchAll(); setShowRemplacementModal(false);
    ...
    <div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'24px'}}>
        ...
        <div style={{position:'relative'}}>
          <button onClick={()=>setShowNotifications(!showNotifications)}
            ...
            {upcomingReservations.length>0&&<span style={{position:'absolute',top:'-3px',right:'-3px',background:RED,color:'white',borderRadius:'50%',width:'17px',height:'17px',fontSize:'10px',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:'800'}}>{upcomingReservations.length}</span>}
          </button>
          {showNotifications&&(
            <div style={{position:'absolute',right:0,top:'50px',background:'white',borderRadius:'12px',width:'340px',boxShadow:'0 8px 32px rgba(0,0,0,0.14)',zIndex:1000,border:'1px solid #DDE3ED',overflow:'hidden'}}>
              <div style={{background:NAVY,color:'white',padding:'13px 16px',fontWeight:'700',fontSize:'14px',display:'flex',alignItems:'center',gap:'8px'}}><Bell size={15}/> Rappels ({upcomingReservations.length})</div>
              {upcomingReservations.length===0
                ...
                : upcomingReservations.map(r=>{
                    const client=clients.find(c=>c.id===r.client), vehicle=vehicles.find(v=>v.id===r.vehicle), color=notifColor(r.daysLeft);
                    return(
                      <div key={r.id} style={{padding:'11px 16px',borderBottom:'1px solid #F0F2F5',borderLeft:`4px solid ${color}`}}>
                        ...
                        <div style={{fontSize:'12px',color:'#64748B',marginTop:'3px',display:'flex',alignItems:'center',gap:'5px'}}><Car size={12}/> {vehicle?`${vehicle.marque} ${vehicle.modele}`:'—'}</div>
                        <div style={{fontSize:'11.5px',color:'#94A3B8',marginTop:'2px',display:'flex',alignItems:'center',gap:'5px'}}><CalendarDays size={11}/> {r.date_debut} → {r.date_fin}</div>
                        <div style={{display:'flex',gap:'6px',marginTop:'7px',flexWrap:'wrap'}}>
                          {r.a_accident&&<button onClick={()=>{setShowNotifications(false);openRemplacement(r);}} style={{padding:'4px 10px',background:RED,color:'white',border:'none',borderRadius:'6px',cursor:'pointer',fontSize:'11px',fontWeight:'700',display:'flex',alignItems:'center',gap:'4px'}}><RefreshCw size={12}/> Remplacer</button>}
                          {canInspect(r)&&<button onClick={()=>{setShowNotifications(false);openInspection(r);}} style={{padding:'4px 10px',background:GREEN,color:'white',border:'none',borderRadius:'6px',cursor:'pointer',fontSize:'11px',fontWeight:'700',display:'flex',alignItems:'center',gap:'4px'}}><ClipboardCheck size={12}/> Inspecter</button>}
                        </div>
                      </div>
                    );
                  })}
            ...
            {aInspecterAujourdhui.map(r=>{
              ...
              return(
                <button key={r.id} onClick={()=>openInspection(r)}
                  style={{padding:'5px 12px',background:GREEN,color:'white',border:'none',borderRadius:'8px',cursor:'pointer',fontWeight:'700',fontSize:'12px',display:'flex',alignItems:'center',gap:'5px'}}>
                  <ClipboardCheck size={12}/> {vehicle?.marque} {vehicle?.modele} · {client?.prenom} {client?.nom}
                </button>
              );
            })}
          </div>
      ...
      <div className="card" style={{marginBottom:'24px'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'20px'}}>
          ...
          {showCalendar&&<div style={{fontSize:'12.5px',color:'#64748B'}}><strong style={{color:NAVY}}>{filteredVehicles.length}</strong> véhicule(s) disponible(s)</div>}
        </div>
        <div style={{display:'grid',gridTemplateColumns:showCalendar?'300px 1fr':'300px',gap:'28px',alignItems:'flex-start'}}>
          <div>
            <div style={{fontSize:'12px',color:'#64748B',marginBottom:'10px',display:'flex',alignItems:'center',gap:'6px'}}>
              <span style={{display:'inline-block',width:'10px',height:'10px',background:'#FCA5A5',borderRadius:'50%'}}></span> Jours réservés
            </div>
            ...
            {calendarDates[0]&&calendarDates[1]&&(
              <div style={{marginTop:'12px',background:'#EFF4FB',borderRadius:'10px',padding:'10px 14px',fontSize:'13px'}}>
                ...
                <div style={{textAlign:'center',fontWeight:'800',color:NAVY,marginTop:'4px'}}>{calcDays(calendarDates[0].toISOString().split('T')[0],calendarDates[1].toISOString().split('T')[0])} jour(s)</div>
                {(()=>{const s=getSaisonInfo(calendarDates[0].toISOString().split('T')[0]);return s?<div style={{marginTop:'8px',background:s.bg,color:s.color,padding:'5px 10px',borderRadius:'7px',fontWeight:'700',fontSize:'11.5px',textAlign:'center'}}>{s.label} ({s.pct})</div>:null;})()}
              </div>
            )}
            <button className="btn btn-primary" onClick={checkAvailability} disabled={checkingAvailability} style={{marginTop:'12px',width:'100%',justifyContent:'center'}}>
          ...
          {showCalendar&&(
            <div>
              <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'14px',flexWrap:'wrap'}}>
                <div style={{display:'flex',alignItems:'center',gap:'5px',color:GREEN,fontWeight:'700',fontSize:'13.5px'}}><CheckCircle size={15}/> Disponibles</div>
                <div style={{position:'relative',flex:1,minWidth:'150px'}}>
                  <Search size={13} color="#94A3B8" style={{position:'absolute',left:'9px',top:'50%',transform:'translateY(-50%)',pointerEvents:'none'}}/>
                  <input value={vehicleSearch} onChange={e=>{setVehicleSearch(e.target.value);setVehiclePage(1);}} placeholder="Marque, modèle..." style={{paddingLeft:'28px',fontSize:'12.5px',padding:'7px 10px 7px 28px'}}/>
                </div>
                <div style={{position:'relative'}}>
                  ...
                  <select value={vehicleSort} onChange={e=>{setVehicleSort(e.target.value);setVehiclePage(1);}} style={{paddingLeft:'28px',fontSize:'12.5px',padding:'7px 10px 7px 28px',minWidth:'130px'}}>
              ...
              {filteredVehicles.length===0
                ? <div style={{background:'#FEE2E2',border:'1px solid #FECACA',borderRadius:'10px',padding:'20px',textAlign:'center',color:RED,fontWeight:'600',fontSize:'13px',display:'flex',flexDirection:'column',alignItems:'center',gap:'8px'}}><AlertTriangle size={22}/>{vehicleSearch?'Aucun véhicule correspond.':'Aucun véhicule disponible pour cette période.'}</div>
                : <>
                    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill, minmax(190px, 1fr))',gap:'12px'}}>
                      {paginatedVehicles.map(v=>{
                        const d1=calendarDates[0].toISOString().split('T')[0];
                        const days=calcDays(d1,calendarDates[1].toISOString().split('T')[0]);
                        const prix=getPrixSaison(v,d1), total=(days*prix).toFixed(2);
                        const mois=calendarDates[0].getMonth()+1, rule=getAcompteRule(days);
                        return(
                          <div key={v.id} style={{background:'white',border:'1.5px solid #DDE3ED',borderRadius:'12px',overflow:'hidden',transition:'box-shadow 0.15s,border-color 0.15s'}}
                            ...
                            <div style={{padding:'10px 12px'}}>
                              ...
                              <div style={{fontSize:'10.5px',color:'#94A3B8',marginBottom:'10px'}}>Acompte min. {rule.pct}% — {(parseFloat(total)*rule.pct/100).toFixed(2)} DT</div>
          ...
          <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:'560px'}}>
            ...
            <div className="form-group">
              ...
              <select id="raison_remplacement" defaultValue="Accident — véhicule indisponible">
                <option>Accident — véhicule indisponible</option><option>Panne mécanique</option>
                <option>Maintenance urgente</option><option>Autre</option>
              </select>
            </div>
                  ...
                  {vehiculesRemplacement.map(v=>(
                    <div key={v.id} style={{padding:'10px 14px',borderRadius:'8px',border:'1px solid #DDE3ED',background:'#F8FAFC',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'10px'}}>
                      <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                        ...
                        <div><div style={{fontWeight:'700',fontSize:'13px'}}>{v.marque} {v.modele}</div><div style={{fontSize:'11.5px',color:'#64748B'}}>{v.immatriculation}</div></div>
                      </div>
                      <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
            ...
            <div className="modal-actions"><button className="btn btn-outline" onClick={()=>setShowRemplacementModal(false)}>Fermer</button></div>
          </div>
        </div>
      )}
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'14px'}}>
                ...
                <div className="form-group">
                  ...
                  {fromCalendar&&selectedVehicle
                    ? <div style={{background:'#DCFCE7',border:`1.5px solid ${GREEN}`,borderRadius:'8px',padding:'8px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                        <div><div style={{fontWeight:'700',fontSize:'13.5px'}}>{selectedVehicle.marque} {selectedVehicle.modele}</div><div style={{fontSize:'11.5px',color:NAVY,marginTop:'1px'}}>{selectedVehicle.immatriculation}</div></div>
                ...
                {form.montant_total&&(
                  <div style={{gridColumn:'1 / -1',background:NAVY,color:'white',borderRadius:'10px',padding:'12px 16px',textAlign:'center'}}>
                    <span style={{fontSize:'13px',opacity:.8}}>Montant total: </span>
                    <strong style={{fontSize:'22px'}}>{form.montant_total} DT</strong>
                    <span style={{fontSize:'13px',opacity:.75,marginLeft:'10px'}}>({formDays} jour{formDays>1?'s':''})</span>
                  </div>
                )}
                <div className="form-group">
      ...
      {showInspection&&inspectionRes&&(
        <RetourCheckModal
          reservation={inspectionRes}
          client={clients.find(c=>c.id===inspectionRes.client)}
          vehicle={vehicles.find(v=>v.id===inspectionRes.vehicle)}
          onClose={()=>{setShowInspection(false);setInspectionRes(null);}}
