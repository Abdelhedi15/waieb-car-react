    ...
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:'16px', paddingTop:'14px', borderTop:'1px solid #DDE3ED' }}>
      ...
      <div style={{ display:'flex', gap:'4px', alignItems:'center' }}>
        <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1}
          style={{ width:'32px', height:'32px', border:'1.5px solid #DDE3ED', borderRadius:'8px',
            background: currentPage === 1 ? '#FAFAFA' : 'white',
            cursor: currentPage === 1 ? 'not-allowed' : 'pointer',
            display:'flex', alignItems:'center', justifyContent:'center',
            color: currentPage === 1 ? '#C8D0DC' : NAVY }}>
          <ChevronLeft size={14}/>
        </button>
        {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
          <button key={p} onClick={() => onPageChange(p)}
            style={{ width:'32px', height:'32px', borderRadius:'8px',
              background: p === currentPage ? NAVY : 'white',
              border: p === currentPage ? 'none' : '1.5px solid #DDE3ED',
              color: p === currentPage ? 'white' : '#1A2535',
              fontWeight: p === currentPage ? '800' : '500', fontSize:'13px', cursor:'pointer' }}>
            {p}
          </button>
        ))}
        <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages}
          style={{ width:'32px', height:'32px', border:'1.5px solid #DDE3ED', borderRadius:'8px',
            ...
            display:'flex', alignItems:'center', justifyContent:'center',
            color: currentPage === totalPages ? '#C8D0DC' : NAVY }}>
...
const Reservations = () => {
  ...
  // ── INSPECTION STATE ──────────────────────────────────────
  const [inspectionRes,  setInspectionRes]  = useState(null); // réservation à inspecter
  ...
  // ── INSPECTION CONFIRM ────────────────────────────────────
  // onConfirm reçoit le payload de RetourCheckModal et fait le PATCH + UPDATE véhicule
  const handleInspectionConfirm = async (payload) => {
    ...
    // 2. UPDATE véhicule : statut=disponible + etat_carrosserie
    const vehicleId = inspectionRes.vehicle;
    const etatCar   = payload.etat_retour === 'excellent' || payload.etat_retour === 'bon'
      ? 'excellent'
      : payload.etat_retour === 'defauts'
      ? 'defauts'
      : 'dommages';

    await api.patch(`/vehicles/${vehicleId}/`, {
      statut:           'disponible',
      etat_carrosserie: etatCar,
      kilometrage:      payload.kilometrage_retour,
    });
  ...
  const checkAvailability = async () => {
    ...
    setVehiclePage(1); setVehicleSearch('');
    try {
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px' }}>
        ...
        <div style={{ position:'relative' }}>
          <button onClick={() => setShowNotifications(!showNotifications)}
            style={{ background:'white', border:'1.5px solid #DDE3ED', borderRadius:'50%', width:'42px', height:'42px',
              display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', position:'relative',
              boxShadow:'0 1px 4px rgba(0,0,0,0.07)' }}>
            <Bell size={19} color={NAVY}/>
            {upcomingReservations.length > 0 && (
              <span style={{ position:'absolute', top:'-3px', right:'-3px', background:RED, color:'white',
          ...
          {showNotifications && (
            <div style={{ position:'absolute', right:0, top:'50px', background:'white', borderRadius:'12px',
              width:'340px', boxShadow:'0 8px 32px rgba(0,0,0,0.14)', zIndex:1000,
              border:'1px solid #DDE3ED', overflow:'hidden' }}>
              <div style={{ background:NAVY, color:'white', padding:'13px 16px', fontWeight:'700', fontSize:'14px',
                display:'flex', alignItems:'center', gap:'8px' }}>
                <Bell size={15}/> Rappels ({upcomingReservations.length})
              </div>
              {upcomingReservations.length === 0 ? (
                <div style={{ padding:'20px', textAlign:'center', color:'#64748B', fontSize:'13px' }}>
                  Aucune location dans les 5 prochains jours
                </div>
                ...
                return (
                  <div key={r.id} style={{ padding:'11px 16px', borderBottom:'1px solid #F0F2F5', borderLeft:`4px solid ${color}` }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                      ...
                      <span style={{ background:color, color:'white', padding:'2px 8px', borderRadius:'10px', fontSize:'10.5px', fontWeight:'800' }}>
                        {notifLabel(r.daysLeft)}
                      </span>
                    </div>
                    ...
                    <div style={{ display:'flex', gap:'6px', marginTop:'7px', flexWrap:'wrap' }}>
                      {r.a_accident && (
                        <button onClick={() => { setShowNotifications(false); openRemplacement(r); }}
                          style={{ padding:'4px 10px', background:RED, color:'white', border:'none', borderRadius:'6px',
                            cursor:'pointer', fontSize:'11px', fontWeight:'700', display:'flex', alignItems:'center', gap:'4px' }}>
                          <RefreshCw size={12}/> Remplacer
                        </button>
                      )}
                      {canInspect(r) && (
                        <button onClick={() => { setShowNotifications(false); openInspection(r); }}
                          style={{ padding:'4px 10px', background:GREEN, color:'white', border:'none', borderRadius:'6px',
                            cursor:'pointer', fontSize:'11px', fontWeight:'700', display:'flex', alignItems:'center', gap:'4px' }}>
                          <ClipboardCheck size={12}/> Inspecter
                        </button>
                      )}
                    </div>
                  </div>
      ...
      {/* ── Réservations à inspecter (badge) */}
      {reservations.filter(r => canInspect(r)).length > 0 && (
        <div style={{ background:'#F0FFF4', border:'2px solid #86EFAC', borderRadius:'12px', padding:'12px 16px',
          marginBottom:'20px', display:'flex', alignItems:'center', gap:'10px', flexWrap:'wrap' }}>
          <ClipboardCheck size={18} color={GREEN}/>
          <span style={{ fontWeight:'700', color:'#166534', fontSize:'13.5px' }}>
            {reservations.filter(r => canInspect(r)).length} réservation(s) à inspecter aujourd'hui ou hier
          </span>
            ...
            {reservations.filter(r => canInspect(r)).map(r => {
              ...
              return (
                <button key={r.id} onClick={() => openInspection(r)}
                  style={{ padding:'5px 12px', background:GREEN, color:'white', border:'none', borderRadius:'8px',
                    cursor:'pointer', fontWeight:'700', fontSize:'12px', display:'flex', alignItems:'center', gap:'5px' }}>
                  <ClipboardCheck size={12}/>
                  {vehicle?.marque} {vehicle?.modele} · {client?.prenom} {client?.nom}
                </button>
              );
            })}
          </div>
          ...
          {showCalendar && (
            <div>
              ...
              {filteredVehicles.length === 0 ? (
                <div style={{ background:'#FEE2E2', border:'1px solid #FECACA', borderRadius:'10px', padding:'20px',
                  textAlign:'center', color:RED, fontWeight:'600', fontSize:'13px',
                        ...
                        <div key={v.id} style={{ background:'white', border:'1.5px solid #DDE3ED', borderRadius:'12px', overflow:'hidden', transition:'box-shadow 0.15s,border-color 0.15s' }}
                          ...
                          <div style={{ padding:'10px 12px' }}>
                            ...
                            <button onClick={() => openReserveFromCalendar(v)}
                              style={{ width:'100%', padding:'8px', background:NAVY, color:'white', border:'none', borderRadius:'8px',
                                cursor:'pointer', fontWeight:'700', fontSize:'12.5px', display:'flex', alignItems:'center', justifyContent:'center', gap:'5px' }}>
                  ...
                  <div key={v.id} style={{ padding:'10px 14px', borderRadius:'8px', border:'1px solid #DDE3ED', background:'#F8FAFC', display:'flex', justifyContent:'space-between', alignItems:'center', gap:'10px' }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                      ...
                      <button onClick={() => confirmerRemplacement(v.id, document.getElementById('raison_remplacement').value)}
                        style={{ padding:'6px 12px', background:NAVY, color:'white', border:'none', borderRadius:'6px',
