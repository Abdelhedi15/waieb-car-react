...
import {
  ...
  Hammer, PenLine, ShieldAlert, HelpCircle, MapPin,
} from 'lucide-react';
...
const IMMAT_PHOTOS = {
  '240TN5082':'https://i.ibb.co/FZmVWK6/vec1.jpg','259TN5651':'https://i.ibb.co/F4SbDBMM/vec2.jpg',
  '243TN1422':'https://i.ibb.co/gbw2JtTH/vec3.jpg','236TN5648':'https://i.ibb.co/0RJ31jBB/vec4.jpg',
  '234TN2126':'https://i.ibb.co/prkyKtjv/vec5.jpg','244TN7005':'https://i.ibb.co/P81vS80/vec6.jpg',
  '251TN1694':'https://i.ibb.co/5WBKGTGL/vec7.jpg','252TN3310':'https://images.unsplash.com/photo-1609521263047-f8f205293f24?w=400&q=80',
  '253TN4421':'https://i.ibb.co/jvRzYcDB/vec9.png','254TN6632':'https://i.ibb.co/hxvysSY4/vec10.png',
  '255TN7743':'https://i.ibb.co/dsfz2VnP/vec11.png','256TN8854':'https://i.ibb.co/35ccmkFY/vec12.jpg',
...
const CATEGORIES = [
  { key:'all',    label:'Tous',             icon:<Car size={13}/> },
  { key:'small',  label:'Petite',           icon:<Car size={13}/> },
  { key:'medium', label:'Moyenne',          icon:<Car size={13}/> },
  { key:'large',  label:'Grande',           icon:<Car size={13}/> },
  { key:'suv',    label:'SUV / 4x4',        icon:<Shield size={13}/> },
  { key:'van',    label:'Van / Utilitaire', icon:<Users size={13}/> },
  { key:'luxury', label:'Luxe / Premium',   icon:<Tag size={13}/> },
];
...
const classifyVehicle = (v) => {
  const model=( v.modele||'').toLowerCase(), brand=(v.marque||'').toLowerCase();
  const places=parseInt(v.nombre_places||5), prix=parseFloat(v.prix_journalier||0);
  ...
  if (places>=6||LARGE_MODELS.some(k=>model.includes(k)))     return 'large';
...
const getReservationStatus = (r) => {
  ...
  if(fin<today)             return{label:'Terminée ⚠',bg:'#DBEAFE',color:'#1B3A6B',isPast:true,staleStatut:true};
  if(debut<=today&&fin>=today) return{label:'Active', bg:'#DCFCE7',color:'#16A34A',isPast:false};
  return {label:'À venir',bg:'#EFF4FB',color:'#1B3A6B',isPast:false};
};

const ZoneChip = ({zones,color,label}) => {
  if(!zones||zones.length===0) return null;
  return <span style={{background:color+'22',color,fontSize:'10.5px',fontWeight:'700',padding:'2px 8px',borderRadius:'8px',display:'inline-flex',gap:'4px',alignItems:'center'}}>{label}: {zones.length}</span>;
};

// ── ✅ INCIDENT TYPES avec Lucide icons ──────────────────────────────────────
const SIG_TYPES = [
  { key:'impact',   label:'Impact / Bosse',    icon:<Hammer size={22}/>,     color:'#DC2626', bg:'#FEF2F2', border:'#FECACA' },
  { key:'rayure',   label:'Éraflure / Rayure', icon:<PenLine size={22}/>,    color:'#D97706', bg:'#FFFBEB', border:'#FDE68A' },
  { key:'accident', label:'Accident',          icon:<ShieldAlert size={22}/>,color:'#7C3AED', bg:'#F5F3FF', border:'#DDD6FE' },
  { key:'autre',    label:'Autre',             icon:<HelpCircle size={22}/>, color:'#64748B', bg:'#F8FAFC', border:'#E2E8F0' },
];
...
// ── incidentIcon pour historique ─────────────────────────────────────────────
const IncidentIcon = ({type, size=16}) => {
  const map = {impact:<Hammer size={size}/>, rayure:<PenLine size={size}/>, accident:<ShieldAlert size={size}/>, autre:<HelpCircle size={size}/>};
  return map[type]||<HelpCircle size={size}/>;
};
const incidentColor = (t) => ({impact:'#DC2626',rayure:'#D97706',accident:'#7C3AED',autre:'#64748B'}[t]||'#64748B');

// ── RENTAL ROW ────────────────────────────────────────────────────────────────
const RentalRow = ({r,idx,onFixStatut}) => {
  const bosses=parseZones(r.bosses_retour), eraflures=parseZones(r.eraflures_retour);
  const etatAv=etatLabel(r.etat_depart||'excellent'), etatAp=etatLabel(r.etat_retour||null);
  ...
  const getInspectionBadge = () => {
    if(r.inspection_retour_faite) return{bg:'#DCFCE7',color:'#16A34A',label:'✓ Inspecté'};
    if(isPast)                    return{bg:'#FEE2E2',color:'#DC2626',label:'⚠ Non inspecté'};
    if(r.statut==='confirmée'||resStatus.label==='Active') return{bg:'#FEF9C3',color:'#D97706',label:'En cours'};
    return {bg:'#EFF4FB',color:'#1B3A6B',label:'À venir'};
  };
  const inspBadge=getInspectionBadge();
  return (
    <div style={{border:`1.5px solid ${hasIssue?'#FECACA':resStatus.staleStatut?'#FCD34D':'#E2E8F0'}`,borderRadius:'10px',marginBottom:'10px',overflow:'hidden',background:hasIssue?'#FFFAFA':resStatus.staleStatut?'#FFFBEB':'white'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'9px 14px',background:hasIssue?'#FFF5F5':resStatus.staleStatut?'#FFFDE7':'#F8FAFC',borderBottom:'1px solid #F1F5F9'}}>
        <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
          <span style={{fontWeight:'800',fontSize:'11px',color:'#1B3A6B',background:'#EFF4FB',borderRadius:'6px',padding:'2px 7px'}}>#{idx+1}</span>
          <Calendar size={12} color="#64748B"/>
          <span style={{fontSize:'12px',fontWeight:'700',color:'#1A2535'}}>{fmtDate(r.date_debut)} → {fmtDate(r.date_fin)}</span>
          <span style={{background:resStatus.bg,color:resStatus.color,padding:'2px 8px',borderRadius:'8px',fontSize:'10px',fontWeight:'700'}}>{resStatus.label}</span>
        </div>
        <div style={{display:'flex',gap:'5px',alignItems:'center'}}>
          {r.a_accident&&<span style={{background:'#FEE2E2',color:'#DC2626',padding:'2px 8px',borderRadius:'8px',fontSize:'10.5px',fontWeight:'700',display:'flex',alignItems:'center',gap:'3px'}}><ShieldAlert size={10}/> Accident</span>}
          <span style={{background:inspBadge.bg,color:inspBadge.color,padding:'2px 8px',borderRadius:'8px',fontSize:'10.5px',fontWeight:'700'}}>{inspBadge.label}</span>
          {resStatus.staleStatut&&onFixStatut&&<button onClick={()=>onFixStatut(r.id)} style={{padding:'2px 9px',background:'#1B3A6B',color:'white',border:'none',borderRadius:'7px',cursor:'pointer',fontSize:'10px',fontWeight:'700'}}>Marquer terminée</button>}
        </div>
      </div>
      <div style={{padding:'11px 14px'}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 20px 1fr',gap:'6px',alignItems:'center',marginBottom:'8px'}}>
          <div style={{background:'#F8FAFC',borderRadius:'8px',padding:'7px 10px'}}>
            ...
            {r.kilometrage_depart!=null&&<div style={{fontSize:'10px',color:'#64748B',marginTop:'4px',display:'flex',gap:'3px',alignItems:'center'}}><Gauge size={9}/> {nb(r.kilometrage_depart).toLocaleString()} km</div>}
          </div>
          ...
          <div style={{background:'#F8FAFC',borderRadius:'8px',padding:'7px 10px'}}>
            ...
            {r.inspection_retour_faite ? (
              <>
                ...
                {r.kilometrage_retour!=null&&<div style={{fontSize:'10px',color:'#64748B',marginTop:'4px',display:'flex',gap:'3px',alignItems:'center',flexWrap:'wrap'}}><Gauge size={9}/> {nb(r.kilometrage_retour).toLocaleString()} km{r.kilometrage_depart!=null&&<span style={{color:'#7C3AED',fontWeight:'700'}}>(+{(nb(r.kilometrage_retour)-nb(r.kilometrage_depart)).toLocaleString()})</span>}</div>}
              </>
            ) : (
              <span style={{fontSize:'10.5px',color:isPast?'#DC2626':'#94A3B8',fontStyle:'italic',fontWeight:isPast?'700':'normal'}}>{isPast?'⚠ Non inspecté':'Pas encore effectué'}</span>
            )}
          </div>
        </div>
        <div style={{display:'flex',gap:'5px',flexWrap:'wrap',alignItems:'center'}}>
          {score&&<span style={{background:score.bg,color:score.color,padding:'2px 9px',borderRadius:'8px',fontSize:'10.5px',fontWeight:'800'}}>Score: {nb(r.score_retour)}/100</span>}
          <ZoneChip zones={bosses}    color="#D97706" label="Bosses"/>
          <ZoneChip zones={eraflures} color="#DC2626" label="Éraflures"/>
          {r.carburant_retour!=null&&<span style={{display:'flex',alignItems:'center',gap:'3px',fontSize:'10px',color:'#64748B',background:'#F8FAFC',padding:'2px 7px',borderRadius:'7px',fontWeight:'600'}}><Fuel size={9}/> Carburant: {r.carburant_retour}%</span>}
        </div>
        {(r.client_nom||r.client)&&<div style={{marginTop:'7px',fontSize:'10.5px',color:'#64748B',display:'flex',gap:'4px',alignItems:'center'}}><User size={10}/> Client: <strong style={{color:'#1A2535'}}>{r.client_nom||`#${r.client}`}</strong></div>}
      </div>
    </div>
  );
};
...
const RapportEtatModal = ({vehicle,reservations,onClose,onDommageSignale}) => {
  ...
  const vRes=[...(reservations||[])].filter(r=>r.vehicle===vehicle?.id).sort((a,b)=>new Date(b.date_debut)-new Date(a.date_debut));

  const handleFixStatut=async(id)=>{
    try{await api.patch(`/reservations/${id}/`,{statut:'terminée'});if(onDommageSignale)onDommageSignale();}catch(err){console.error(err);}
  };

  useEffect(()=>{
    if(!vehicle?.id)return;
    setLoadingInc(true);
    api.get(`/incidents/?vehicle=${vehicle.id}`).then(res=>setIncidents(res.data)).catch(()=>setIncidents([])).finally(()=>setLoadingInc(false));
  },[vehicle?.id,sigDone]);

  const today=new Date();today.setHours(0,0,0,0);
  const locations=vRes.length;
  const sinistres=[...vRes.filter(r=>r.a_accident),...incidents.filter(i=>i.type_incident==='accident')].length;
  const impacts=vRes.reduce((s,r)=>s+parseZones(r.bosses_retour).length,0)+incidents.filter(i=>i.type_incident==='impact').length;
  const rayures=vRes.reduce((s,r)=>s+parseZones(r.eraflures_retour).length,0)+incidents.filter(i=>i.type_incident==='rayure').length;
  const pastRes=vRes.filter(r=>{const f=new Date(r.date_fin);f.setHours(0,0,0,0);return f<today||r.statut==='terminée';});
  const inspected=pastRes.filter(r=>r.inspection_retour_faite).length;
  const kmTotal=vRes.reduce((s,r)=>{const d=nb(r.kilometrage_retour)-nb(r.kilometrage_depart);return s+(d>0?d:0);},0);
  ...
  const handleMarkRepare=async(incId)=>{
    try{await api.patch(`/incidents/${incId}/`,{repare:true,date_reparation:new Date().toISOString().split('T')[0]});setIncidents(prev=>prev.map(i=>i.id===incId?{...i,repare:true}:i));}
    catch{alert('Erreur lors de la mise à jour');}
  };

  return (
    <div className="modal-overlay" onClick={onClose} style={{zIndex:1100}}>
      <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:'680px',maxHeight:'90vh',display:'flex',flexDirection:'column',padding:0}}>

        {/* Header */}
        <div style={{padding:'16px 20px',background:NAVY,borderRadius:'12px 12px 0 0',display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0}}>
          <div>
          ...
          <div style={{display:'flex',gap:'8px'}}>
            <button onClick={()=>{setShowSignal(s=>!s);setSigDone(false);}}
              style={{background:showSignal?'#DC2626':'rgba(220,38,38,0.18)',color:showSignal?'white':'#FCA5A5',border:'none',borderRadius:'8px',cursor:'pointer',padding:'7px 13px',fontWeight:'700',fontSize:'12px',display:'flex',alignItems:'center',gap:'5px'}}>
              <AlertTriangle size={14}/> {showSignal?'✕ Fermer':'+ Signaler dommage'}
            </button>
            <button onClick={onClose} style={{background:'rgba(255,255,255,0.15)',border:'none',borderRadius:'8px',color:'white',cursor:'pointer',padding:'6px',display:'flex'}}><X size={17}/></button>
          </div>
        </div>

        {/* ✅ Panneau signalement avec Lucide icons */}
        {showSignal && (
          <div style={{background:'#FFF5F5',borderBottom:'2px solid #FECACA',padding:'16px 20px',flexShrink:0}}>
            {sigDone ? (
              <div style={{textAlign:'center',padding:'14px',color:'#16A34A',fontWeight:'800',fontSize:'14px',background:'#F0FFF4',borderRadius:'10px',border:'2px solid #86EFAC'}}>
                ✅ Incident enregistré — état du véhicule mis à jour automatiquement
              </div>
            ) : (
              <>
                <div style={{fontWeight:'800',fontSize:'12.5px',color:'#DC2626',marginBottom:'12px',display:'flex',alignItems:'center',gap:'6px'}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px',marginBottom:'10px'}}>
                  <div>
                    ...
                    <select value={sigZone} onChange={e=>setSigZone(e.target.value)} style={{width:'100%',padding:'7px',borderRadius:'7px',border:'1.5px solid #DDE3ED',fontSize:'12px'}}>
                  ...
                  <div>
                    ...
                    <input type="date" value={sigDate} onChange={e=>setSigDate(e.target.value)} style={{width:'100%',padding:'7px',borderRadius:'7px',border:'1.5px solid #DDE3ED',fontSize:'12px',boxSizing:'border-box'}}/>
                  </div>
                  <div>
                    <label style={{fontSize:'10.5px',fontWeight:'700',color:'#64748B',display:'block',marginBottom:'3px'}}>Coût réparation (DT)</label>
                    <input type="number" value={sigCout} onChange={e=>setSigCout(e.target.value)} placeholder="Optionnel" style={{width:'100%',padding:'7px',borderRadius:'7px',border:'1.5px solid #DDE3ED',fontSize:'12px',boxSizing:'border-box'}}/>
                  </div>
                </div>
                ...
                <div style={{marginBottom:'10px'}}>
                  <label style={{fontSize:'10.5px',fontWeight:'700',color:'#64748B',display:'block',marginBottom:'3px'}}>Description <span style={{color:'#DC2626'}}>*</span></label>
                  <textarea value={sigNotes} onChange={e=>setSigNotes(e.target.value)} placeholder="Ex: Éraflure profonde sur le flanc droit..." rows={2}
                    style={{width:'100%',padding:'9px',border:'1.5px solid #FECACA',borderRadius:'8px',fontSize:'12.5px',resize:'none',fontFamily:'inherit',boxSizing:'border-box'}}/>
                </div>

                <div style={{background:'#FEF9C3',border:'1px solid #FDE68A',borderRadius:'8px',padding:'7px 12px',fontSize:'11px',color:'#92580A',fontWeight:'600',marginBottom:'10px',display:'flex',alignItems:'center',gap:'6px'}}>
                  <AlertTriangle size={12}/> L'état carrosserie du véhicule sera mis à jour automatiquement.
                </div>

                <button onClick={handleSignaler} disabled={sigLoading||!sigNotes.trim()}
                  style={{width:'100%',padding:'10px',fontWeight:'800',fontSize:'13px',border:'none',borderRadius:'9px',
                    cursor:sigLoading||!sigNotes.trim()?'not-allowed':'pointer',
                  ...
                  {sigLoading ? <><Wrench size={14}/> Enregistrement...</> : <><AlertTriangle size={14}/> Enregistrer l'incident</>}
                </button>
              </>
            )}
          </div>
        ...
        <div style={{overflowY:'auto',flex:1,padding:'16px 20px'}}>
          ...
          {nonInspectedPast>0&&(
            <div style={{background:'#FEF9C3',border:'1.5px solid #FCD34D',borderRadius:'10px',padding:'10px 14px',marginBottom:'14px',display:'flex',gap:'8px',alignItems:'center'}}>
              <AlertTriangle size={14} color="#D97706"/>
              <span style={{fontSize:'12px',color:'#92580A',fontWeight:'700'}}>{nonInspectedPast} réservation(s) passée(s) sans inspection de retour</span>
            </div>
          )}

          {/* État actuel */}
          <div style={{background:'#F8FAFC',borderRadius:'10px',padding:'12px 14px',marginBottom:'14px',border:'1.5px solid #E2E8F0'}}>
            <div style={{fontWeight:'800',fontSize:'12.5px',color:NAVY,marginBottom:'8px',display:'flex',gap:'5px',alignItems:'center'}}>
              <Eye size={13}/> État actuel du véhicule
            </div>
            <div style={{display:'flex',gap:'8px',flexWrap:'wrap',alignItems:'center'}}>
              <span style={{background:et.bg,color:et.color,padding:'2px 9px',borderRadius:'9px',fontSize:'11px',fontWeight:'700'}}>{et.label}</span>
              <span style={{fontSize:'11px',color:'#64748B',display:'flex',gap:'3px',alignItems:'center'}}><Gauge size={11}/> {nb(vehicle.kilometrage).toLocaleString()} km</span>
              <span style={{fontSize:'11px',color:'#64748B',display:'flex',gap:'3px',alignItems:'center'}}><Fuel size={11}/> {vehicle.type_carburant}</span>
              <span style={{fontSize:'11px',color:'#64748B',display:'flex',gap:'3px',alignItems:'center'}}><Users size={11}/> {vehicle.nombre_places} places</span>
            </div>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'7px',marginBottom:'12px'}}>
            {[
              ...
              {icon:<ShieldAlert size={14}/>,   val:sinistres,label:'Sinistres',color:'#DC2626' },
              {icon:<Hammer size={14}/>,        val:impacts,  label:'Impacts',  color:'#D97706' },
              {icon:<PenLine size={14}/>,       val:rayures,  label:'Rayures',  color:'#7C3AED' },
            ].map(s=>(
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'7px',marginBottom:'16px'}}>
            ...
            <div style={{background:'#F3EEFF',borderRadius:'8px',padding:'9px 10px',textAlign:'center'}}>
              <div style={{fontSize:'9.5px',color:'#64748B',fontWeight:'700',textTransform:'uppercase',marginBottom:'3px'}}>Score moyen</div>
            ...
            <div style={{background:nonInspectedPast>0?'#FEF9C3':'#DCFCE7',borderRadius:'8px',padding:'9px 10px',textAlign:'center'}}>
              ...
              <div style={{fontWeight:'800',fontSize:'14px',color:nonInspectedPast>0?'#D97706':'#16A34A'}}>{inspected}/{pastRes.length}</div>
            </div>
          </div>
          ...
          {(incidents.length>0||loadingInc)&&(
            <div style={{marginBottom:'16px'}}>
              <div style={{fontWeight:'800',fontSize:'12.5px',color:'#DC2626',marginBottom:'9px',display:'flex',gap:'5px',alignItems:'center'}}>
                <AlertTriangle size={13}/> Incidents enregistrés
                <span style={{background:'#FEE2E2',color:'#DC2626',padding:'1px 7px',borderRadius:'7px',fontSize:'10.5px'}}>{incidents.length}</span>
              </div>
              {loadingInc ? <div style={{textAlign:'center',padding:'12px',color:'#94A3B8',fontSize:'12px'}}>Chargement...</div>
              : incidents.map(inc=>(
                <div key={inc.id} style={{border:`1.5px solid ${inc.repare?'#DDE3ED':incidentColor(inc.type_incident)+'55'}`,borderRadius:'9px',marginBottom:'8px',overflow:'hidden',background:inc.repare?'#F8FAFC':'#FFFAFA',opacity:inc.repare?0.7:1}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 12px',background:inc.repare?'#F1F5F9':incidentColor(inc.type_incident)+'10',borderBottom:'1px solid #F1F5F9'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'7px'}}>
                      <div style={{color:incidentColor(inc.type_incident),display:'flex'}}><IncidentIcon type={inc.type_incident} size={15}/></div>
                      <span style={{fontWeight:'800',fontSize:'11.5px',color:incidentColor(inc.type_incident)}}>{inc.type_label}</span>
                      <span style={{background:incidentColor(inc.type_incident)+'20',color:incidentColor(inc.type_incident),padding:'1px 7px',borderRadius:'7px',fontSize:'10px',fontWeight:'700'}}>{inc.gravite_label}</span>
                      {inc.zone&&<span style={{background:'#F1F5F9',color:'#64748B',padding:'1px 7px',borderRadius:'7px',fontSize:'10px',fontWeight:'600',display:'flex',alignItems:'center',gap:'3px'}}><MapPin size={9}/> {inc.zone}</span>}
                    </div>
                    <div style={{display:'flex',alignItems:'center',gap:'6px'}}>
                      <span style={{fontSize:'10.5px',color:'#94A3B8',fontWeight:'600'}}>{new Date(inc.date_incident).toLocaleDateString('fr-FR')}</span>
                      {inc.repare
                        ? <span style={{background:'#DCFCE7',color:'#16A34A',padding:'2px 8px',borderRadius:'7px',fontSize:'10px',fontWeight:'700',display:'flex',alignItems:'center',gap:'3px'}}><CheckCircle size={10}/> Réparé</span>
                        : <button onClick={()=>handleMarkRepare(inc.id)} style={{background:'#DCFCE7',color:'#16A34A',border:'none',borderRadius:'7px',padding:'3px 9px',cursor:'pointer',fontWeight:'700',fontSize:'10px',display:'flex',alignItems:'center',gap:'3px'}}><CheckCircle size={10}/> Marquer réparé</button>}
                    </div>
                  </div>
                  <div style={{padding:'8px 12px'}}>
                    ...
                    <div style={{display:'flex',gap:'10px',flexWrap:'wrap'}}>
                      {inc.reservation_info
                        ? <span style={{fontSize:'10.5px',color:'#1B3A6B',display:'flex',gap:'3px',alignItems:'center'}}><Calendar size={10}/> Rés. #{inc.reservation_info.id} · {inc.reservation_info.date_debut} → {inc.reservation_info.date_fin}</span>
                        : <span style={{fontSize:'10.5px',color:'#94A3B8',fontStyle:'italic',display:'flex',alignItems:'center',gap:'3px'}}><Wrench size={10}/> Incident hors réservation (garage)</span>}
                      {inc.cout_reparation&&<span style={{fontSize:'10.5px',color:'#D97706',fontWeight:'700'}}>Coût: {parseFloat(inc.cout_reparation).toFixed(0)} DT</span>}
                    </div>
                  </div>
                </div>
              ))}
          ...
          {/* Historique */}
          <div style={{fontWeight:'800',fontSize:'12.5px',color:NAVY,marginBottom:'9px',display:'flex',gap:'5px',alignItems:'center'}}>
          ...
          {vRes.length===0
            ? <div style={{textAlign:'center',padding:'28px',color:'#94A3B8',background:'#F8FAFC',borderRadius:'10px',border:'1.5px dashed #DDE3ED'}}><Car size={30} color="#DDE3ED" style={{margin:'0 auto 8px'}}/><p style={{fontWeight:'600',color:'#64748B',margin:0}}>Aucune location enregistrée</p></div>
            : vRes.map((r,idx)=><RentalRow key={r.id} r={r} idx={vRes.length-1-idx} onFixStatut={handleFixStatut}/>)
          }
        </div>

        {/* Footer */}
        <div style={{padding:'11px 20px',borderTop:'1px solid #E2E8F0',background:'#F8FAFC',borderRadius:'0 0 12px 12px',flexShrink:0,display:'flex',justifyContent:'flex-end'}}>
          <button onClick={onClose} style={{padding:'8px 22px',background:NAVY,color:'white',border:'none',borderRadius:'8px',cursor:'pointer',fontWeight:'700',fontSize:'13px'}}>Fermer</button>
        </div>
      </div>
    </div>
  );
...
const Vehicles = () => {
  ...
  const hasAlert=(v)=>{
    ...
    const soon=(d)=>{if(!d)return false;const dt=new Date(d);return dt<today||((dt-today)<30*24*3600*1000);};
  ...
  const openEdit=(v)=>{setEditingVeh(v);setForm({...EMPTY_FORM,...v,prix_journalier:v.prix_journalier||'',prix_haute_saison:v.prix_haute_saison||'',prix_tres_haute_saison:v.prix_tres_haute_saison||''});setActiveTab('infos');setShowModal(true);};
  ...
  const NAVY='#1B3A6B', AMBER='#E8A020';
  ...
  return (
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
        ...
        {isAdmin&&<button className="btn btn-primary" onClick={openAdd} style={{display:'flex',alignItems:'center',gap:'7px'}}><Plus size={16}/> Ajouter un véhicule</button>}
      </div>
        ...
        {CATEGORIES.map(cat=>{
          ...
          return(
            <button key={cat.key} onClick={()=>{setCatFilter(cat.key);setCurrentPage(1);}}
              ...
              {cat.icon} {cat.label}
              <span style={{background:active?'rgba(255,255,255,0.22)':'#F1F5F9',color:active?'white':'#64748B',padding:'1px 7px',borderRadius:'10px',fontSize:'11px',fontWeight:'700'}}>{count}</span>
            </button>
          );
        })}
      </div>
      ...
      <div style={{display:'flex',gap:'12px',marginBottom:'12px',flexWrap:'wrap'}}>
        <div style={{position:'relative',flex:1,minWidth:'200px'}}>
          ...
          <input value={search} onChange={e=>{setSearch(e.target.value);setCurrentPage(1);}} placeholder="Rechercher par marque, modèle, immatriculation..." style={{paddingLeft:'34px',width:'100%'}}/>
        </div>
      </div>
        ...
        {STATUT_FILTERS.map(s=>{
          ...
          return(
            <button key={s.value} onClick={()=>{setStatFilter(s.value);setCurrentPage(1);}}
              ...
              {s.label}
              {count!==null&&<span style={{background:active?'rgba(255,255,255,0.25)':'rgba(0,0,0,0.08)',padding:'1px 7px',borderRadius:'10px',fontSize:'11px',fontWeight:'800'}}>{count}</span>}
            </button>
          );
        })}
      </div>
      ...
      {paginated.length===0 ? (
        <div className="card" style={{textAlign:'center',padding:'40px',color:'#64748B'}}>
          <Car size={40} color="#DDE3ED" style={{margin:'0 auto 12px'}}/><p style={{fontWeight:'600'}}>Aucun véhicule trouvé</p>
        </div>
      ) : (
            ...
            return(
              <div key={v.id} style={{background:'white',borderRadius:'14px',border:`1.5px solid ${alert?'#FECACA':'#DDE3ED'}`,overflow:'hidden',transition:'box-shadow 0.15s',boxShadow:'0 1px 4px rgba(0,0,0,0.06)'}}
                ...
                <div style={{position:'relative',height:'150px',overflow:'hidden'}}>
                  ...
                  {alert&&<div style={{position:'absolute',top:'10px',right:'10px'}}><AlertTriangle size={18} color="#DC2626" style={{background:'white',borderRadius:'50%',padding:'2px'}}/></div>}
                  <div style={{position:'absolute',bottom:'8px',right:'8px',display:'flex',flexDirection:'column',gap:'3px',alignItems:'flex-end'}}>
                    {v.prix_haute_saison&&<span style={{background:'#FEF3DC',color:'#92580A',padding:'2px 7px',borderRadius:'8px',fontSize:'10px',fontWeight:'700'}}>Été: {parseFloat(v.prix_haute_saison).toFixed(0)} DT/j</span>}
                    {v.prix_tres_haute_saison&&<span style={{background:'#FEE2E2',color:'#DC2626',padding:'2px 7px',borderRadius:'8px',fontSize:'10px',fontWeight:'700'}}>Juil-Août: {parseFloat(v.prix_tres_haute_saison).toFixed(0)} DT/j</span>}
                  </div>
                </div>
                <div style={{padding:'14px 16px'}}>
                  ...
                  <div style={{display:'flex',gap:'12px',flexWrap:'wrap',marginBottom:'10px'}}>
                    {[{icon:<Fuel size={12}/>,val:v.type_carburant},{icon:<Users size={12}/>,val:`${v.nombre_places} pl.`},{icon:<Gauge size={12}/>,val:`${(v.kilometrage||0).toLocaleString()} km`},{icon:<Palette size={12}/>,val:v.couleur}].map((s,i)=>(
                      <span key={i} style={{display:'flex',alignItems:'center',gap:'4px',fontSize:'11.5px',color:'#64748B'}}>{s.icon} {s.val}</span>
                    ))}
                  </div>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',background:'#F8FAFC',borderRadius:'8px',padding:'7px 10px',marginBottom:'10px'}}>
                    <span style={{fontSize:'11.5px',color:'#64748B',display:'flex',alignItems:'center',gap:'5px'}}><Shield size={12}/> État carrosserie</span>
                    <span style={{background:et.bg,color:et.color,padding:'2px 8px',borderRadius:'8px',fontSize:'11px',fontWeight:'700'}}>{et.label}</span>
                  </div>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'4px',marginBottom:'12px'}}>
                    {[
                      ...
                      {icon:<ShieldAlert size={13}/>,  val:stats.sinistres,label:'Sinistres',color:'#DC2626' },
                      {icon:<Hammer size={13}/>,       val:stats.impacts,  label:'Impacts',  color:'#D97706' },
                      {icon:<PenLine size={13}/>,      val:stats.rayures,  label:'Rayures',  color:'#7C3AED' },
                    ].map(s=>(
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:'7px'}}>
                    <div style={{display:'flex',gap:'7px'}}>
                      <button onClick={()=>openEdit(v)} style={{flex:1,padding:'8px',background:NAVY,color:'white',border:'none',borderRadius:'8px',cursor:'pointer',fontWeight:'700',fontSize:'12.5px',display:'flex',alignItems:'center',justifyContent:'center',gap:'5px'}}><Pencil size={13}/> Modifier</button>
          ...
          <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:'700px'}}>
            ...
            <div style={{display:'flex',gap:'4px',marginBottom:'20px',background:'#F8FAFC',borderRadius:'10px',padding:'4px'}}>
              {[{key:'infos',label:'Infos'},{key:'tech',label:'Technique'},{key:'tarifs',label:'Tarifs'},{key:'assurance',label:'Assurance'},{key:'photo',label:'Photo'}].map(t=>(
                <button key={t.key} onClick={()=>setActiveTab(t.key)} style={{flex:1,padding:'7px',border:'none',borderRadius:'8px',cursor:'pointer',fontWeight:'700',fontSize:'12.5px',background:activeTab===t.key?NAVY:'transparent',color:activeTab===t.key?'white':'#64748B'}}>{t.label}</button>
              ))}
            </div>
            <form onSubmit={handleSubmit}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'14px'}}>
                  ...
                  <div className="form-group"><label>Statut</label>
                    <select value={form.statut} onChange={e=>setForm({...form,statut:e.target.value})}>
                      <option value="disponible">Disponible</option><option value="loué">Loué</option>
                      <option value="maintenance">Maintenance</option><option value="a_vendre">À vendre</option>
                      <option value="vendu">Vendu</option><option value="hors service">Hors service</option>
                    </select>
                  </div>
                  <div className="form-group" style={{gridColumn:'1/-1'}}><label>État carrosserie</label>
                    <select value={form.etat_carrosserie} onChange={e=>setForm({...form,etat_carrosserie:e.target.value})}>
                      <option value="excellent">Excellent état</option><option value="defauts">Défauts mineurs</option>
                      <option value="dommages">Dommages visibles</option><option value="sinistre">Sinistre déclaré</option>
                    </select>
                  </div>
                </div>
              )}
              ...
              {activeTab==='tarifs'&&(
                <div>
                  <div className="form-group"><label>Prix journalier de base (DT) *</label><input type="number" value={form.prix_journalier} onChange={e=>setForm({...form,prix_journalier:e.target.value})} required step="0.01"/></div>
                  <button type="button" onClick={suggestPrices} style={{marginBottom:'14px',padding:'8px 14px',background:'#EFF4FB',color:NAVY,border:'1.5px solid #DDE3ED',borderRadius:'8px',cursor:'pointer',fontWeight:'700',fontSize:'12.5px'}}>Suggérer prix saisonniers automatiquement</button>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'14px'}}>
              ...
              {activeTab==='photo'&&(
                <div>
                  {editingVeh?.photo&&<div style={{marginBottom:'14px'}}><img src={getCarPhoto(editingVeh)} alt="actuelle" style={{width:'100%',height:'180px',objectFit:'cover',borderRadius:'10px'}}/><p style={{fontSize:'12px',color:'#64748B',marginTop:'6px'}}>Photo actuelle</p></div>}
                  <div className="form-group"><label>Choisir une nouvelle photo</label><input type="file" accept="image/*" onChange={e=>setPhotoFile(e.target.files[0])}/></div>
                  {photoFile&&<img src={URL.createObjectURL(photoFile)} alt="aperçu" style={{width:'100%',height:'160px',objectFit:'cover',borderRadius:'10px',marginTop:'10px'}}/>}
                </div>
              )}
              <div className="modal-actions">
                <button type="button" className="btn btn-outline" onClick={()=>setShowModal(false)}>Annuler</button>
                <button type="submit" className="btn btn-primary" disabled={loading}>{loading?'Enregistrement...':(editingVeh?'Modifier':'Ajouter')}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {rapportVeh&&<RapportEtatModal vehicle={rapportVeh} reservations={reservations} onClose={()=>setRapportVeh(null)} onDommageSignale={fetchAll}/>}
    </div>
  );
};
